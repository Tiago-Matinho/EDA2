...
struct path* dijkstra(struct air_hash* air_hash, struct fly_hash* fly_hash,
    ...
    char teste[FLIGHT_CODE] = "MQ969\0";
    char teste2[FLIGHT_CODE] = "KR15\0\0";
    ...
    while(!dist_list_empty(vertice_queue)){
        ...
        if(strcmp(vertice1->name, start_name) == 0)
            ...
            GMT_dif = airport2->GMT - GMT_old;
            arrival_time = (short) (dep_time + vertice1->distance + (GMT_dif));//falta aqui o GMT
        }
        ...
        while(current_flight != NULL){ //FOR ADJ U
            ...
            //if(strcmp(current_flight->name, teste) == 0 || strcmp(current_flight->name, teste2) == 0)
            //    printf("teste\n");
            ...
            if(arrival_time + time <= current_flight->dep_time){
                time = current_flight->dep_time - arrival_time; //tempo de espera
                time += current_flight->duration;               //duração do voo
            }
...
int main(){
    ...
    char teste[AIRPORT] = "LGH\0";
    char teste2[AIRPORT] = "BDD\0";
    ...
    while(flag){
        ...
        if(strcmp(command, "AI") == 0){
            ...
            //if(strcmp(airport1, teste) == 0 && strcmp(airport2, teste2) == 0)
            //    printf("teste\n");
